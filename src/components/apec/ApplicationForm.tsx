...
const FEATURES = [
  {
    ...
    description:
      "Благодаря нашему опыту практически все наши клиенты получают карту АТЭС",
  },
  {
    icon: "Clock3",
    title: "Экономия времени",
    description:
      "Мы берем на себя всю рутинную работу по подготовке и подаче документов",
  },
  {
    icon: "FileText",
    title: "Юридическая поддержка",
    description:
      "Наши специалисты помогут правильно составить все необходимые документы",
  },
  {
    icon: "HeadsetHelp",
    title: "Персональный менеджер",
    description:
      "Вы будете иметь персонального менеджера на всех этапах оформления",
  },
];
...
const TestimonialCard = ({
  ...
  <div
    className="bg-white p-6 rounded-xl shadow-md border border-gray-100 relative animate-fade-in-up"
    aria-label={`Отзыв от ${author}`}
  >
...
const FormHeader = () => (
  <CardHeader>
    ...
    <CardDescription>
      Укажите ваши данные, и мы свяжемся с вами для консультации по оформлению
      карты АТЭС
    </CardDescription>
  </CardHeader>
);
...
const PersonalInfoFields = () => (
  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
    <FormField id="firstName" label="Имя" placeholder="Иван" required />
    <FormField id="lastName" label="Фамилия" placeholder="Иванов" required />
  </div>
);
  ...
  <>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
      <FormField
        ...
        required
      />
      <FormField id="tariff" label="Интересующий тариф" required>
        <select
          id="tariff"
...
const MessageField = () => (
  <FormField id="message" label="Дополнительная информация">
    <Textarea
      id="message"
  ...
  <div className="lg:w-5/12">
    <div className="sticky top-20">
      <h2 className="text-4xl font-bold mb-6">
        Оформите <span className="gradient-text">карту АТЭС</span> с нашими
        экспертами
      </h2>
      <p className="text-lg text-muted-foreground mb-8">
        Мы предлагаем полное сопровождение от проверки соответствия требованиям
        до получения готового документа. Заполните форму, и наш эксперт свяжется
        с вами.
      </p>
