...
const Testimonials = () => {
  ...
  // Обработчик перехода к определенному отзыву
  const goToTestimonial = (index: number) => {
    setActiveIndex(index);
    
    // Приостанавливаем автопроигрывание на некоторое время после ручного переключения
    setAutoPlay(false);
    setTimeout(() => setAutoPlay(true), 10000);
  };
  
  // Переход к следующему/предыдущему отзыву
  const nextTestimonial = () => goToTestimonial((activeIndex + 1) % testimonials.length);
  const prevTestimonial = () => goToTestimonial((activeIndex - 1 + testimonials.length) % testimonials.length);
  
  // Получаем текущий отзыв
  const currentTestimonial = testimonials[activeIndex];
  
  // Определяем предыдущий и следующий отзывы для превью
  ...
  return (
    <section id="testimonials" className="py-24 relative overflow-hidden bg-white">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        {/* Основной слайдер с отзывами */}
        <div className="max-w-5xl mx-auto">
          {/* Карточка с текущим отзывом */}
              ...
              <CardContent className="p-8 md:p-10">
                <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
                  ...
                  <div className="md:col-span-4 flex flex-col items-center md:items-start">
                    ...
                    <h3 className="text-xl font-bold text-center md:text-left">{currentTestimonial.name}</h3>
                    <p className="text-muted-foreground text-center md:text-left">{currentTestimonial.position}</p>
                    <p className="text-sm font-medium text-primary mt-1 text-center md:text-left">{currentTestimonial.company}</p>
                    ...
                    {currentTestimonial.businessType && (
                      <div className="flex items-center mt-2 text-sm text-muted-foreground">
                        <Icon name="Briefcase" className="h-4 w-4 mr-1" />
          ...
          {/* Превью предыдущего и следующего отзывов */}
